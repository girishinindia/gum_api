...
export async function create(req: Request, res: Response) {
  ...
  if (file) {
    ...
    const cdnPath = `assessments/${assessment.slug}/attachments/${ts}_${safeName}`;
    try {
...
export async function update(req: Request, res: Response) {
  ...
  const { data: old } = await supabase.from('assessment_attachments').select('*, assessments(slug)').eq('id', id).single();
  ...
  if (file) {
    ...
    const ts = Date.now();
    const safeName = file.originalname.replace(/[^a-zA-Z0-9._-]/g, '_');
    const cdnPath = `assessments/${slug}/attachments/${ts}_${safeName}`;
    try {
...
export async function remove(req: Request, res: Response) {
  const id = parseInt(req.params.id);
  const { data: old } = await supabase.from('assessment_attachments').select('assessment_id, file_name, file_url').eq('id', id).single();
  if (!old) return err(res, 'Assessment attachment not found', 404);
