...
export async function create(req: Request, res: Response) {
  ...
  if (imageUrl) logData({ actorId: req.user!.id, action: 'media_uploaded', resourceType: 'sub_category_translation', resourceId: data.id, resourceName: data.name, ip: getClientIp(req), metadata: { type: 'sub_category_translation_image' } });
...
export async function update(req: Request, res: Response) {
  ...
  if (req.file) logData({ actorId: req.user!.id, action: 'media_uploaded', resourceType: 'sub_category_translation', resourceId: id, resourceName: data.name, ip: getClientIp(req), metadata: { type: 'sub_category_translation_image', old_url: old.image } });
...
export async function remove(req: Request, res: Response) {
  const id = parseInt(req.params.id);
  const { data: old } = await supabase.from('sub_category_translations').select('name, image, sub_category_id').eq('id', id).single();
  if (!old) return err(res, 'Sub-category translation not found', 404);
