...
// deterministic path `course-modules/translations/<tid>/<segment>.webp`.
// Icon uses ICON_BOX_PX (256); the rest use IMAGE_BOX_PX (512).

export type CourseModuleTranslationImageSlot = 'icon' | 'image' | 'ogImage' | 'twitterImage';
export type CourseModuleTranslationImageFiles = Partial<
  Record<CourseModuleTranslationImageSlot, Express.Multer.File>
>;

const COURSE_MODULE_TRANSLATION_SLOT_CONFIG: Record<
  CourseModuleTranslationImageSlot,
...
const setCourseModuleTranslationImageUrl = async (
  id: number,
  slot: CourseModuleTranslationImageSlot,
  url: string | null,
  callerId: number | null
): Promise<void> => {
  await db.callFunction('udf_update_course_module_translations', {
...
export const processCourseModuleTranslationImageUploads = async (
  translationId: number,
  files: CourseModuleTranslationImageFiles,
  callerId: number | null
): Promise<void> => {
  const existing = await getCourseModuleTranslationById(translationId);
  if (!existing) {
    throw AppError.notFound(`Course module translation ${translationId} not found`);
  }

  const currentUrls: Record<CourseModuleTranslationImageSlot, string | null> = {
    icon: existing.icon,
  ...
  const slots: CourseModuleTranslationImageSlot[] = ['icon', 'image', 'ogImage', 'twitterImage'];
  for (const slot of slots) {
    const file = files[slot];
    if (!file) continue;
    const cfg = COURSE_MODULE_TRANSLATION_SLOT_CONFIG[slot];
    const result = await replaceImage({
      inputBuffer: file.buffer,
      targetPath: `course-modules/translations/${translationId}/${cfg.pathSegment}.webp`,
      currentUrl: currentUrls[slot],
      boxPx: cfg.boxPx,
      maxBytes: IMAGE_MAX_BYTES,
      logContext: { courseModuleTranslationId: translationId, slot }
    });
    if (!result) {
      throw AppError.badRequest(
        `Course module translation ${slot} is too complex to compress under ${Math.round(IMAGE_MAX_BYTES / 1024)} KB. Try a simpler image.`,
        { slot, maxBytes: IMAGE_MAX_BYTES }
      );
    }
    await setCourseModuleTranslationImageUrl(translationId, slot, result.cdnUrl, callerId);
  }
};
