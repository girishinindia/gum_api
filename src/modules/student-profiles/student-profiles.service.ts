...
// ─── Resume uploads (Bunny CDN) ──────────────────────────────────
//
// Pipeline: multer buffer → determine MIME type → upload to Bunny
// → udf_update_student_profiles with the resulting CDN URL.
//
// Key rules baked in here (NOT in the route):
//   • storage keys are timestamp-suffixed (`student-profiles/{studentId}/resume-{timestamp}.{ext}`)
...
//   • No transformation — keep original bytes (PDFs, resumes, documents)
 ...
 * files we own — if a resume URL is pointing at an external URL we
...
const safeDeleteFromBunny = async (
  ...
  try {
    ...
    logger.warn(
      { err, path, profileId },
      'Student profile resume: pre-upload delete failed (object may not exist); continuing with new upload'
    );
  }
};

/**
 * Internal-only: persist a new resume URL after a successful Bunny upload.
 * Not exported via any route — the upload endpoint is the single entry point.
 */
const setStudentProfileResume = async (
  id: number,
  resumeUrl: string,
  callerId: number | null
): Promise<void> => {
  await db.callFunction('udf_update_student_profiles', {
    p_id: id,
    p_enrollment_number: null,
    p_enrollment_date: null,
    p_enrollment_type: null,
    p_education_level_id: null,
    p_current_institution: null,
    p_current_field_of_study: null,
    p_current_semester_or_year: null,
    p_expected_graduation_date: null,
    p_is_currently_studying: null,
    p_learning_goal_id: null,
    p_specialization_id: null,
    p_preferred_learning_mode: null,
    p_preferred_learning_language_id: null,
    p_preferred_content_type: null,
    p_daily_learning_hours: null,
    p_weekly_available_days: null,
    p_difficulty_preference: null,
    p_parent_guardian_name: null,
    p_parent_guardian_phone: null,
    p_parent_guardian_email: null,
    p_parent_guardian_relation: null,
    p_courses_enrolled: null,
    p_courses_completed: null,
    p_courses_in_progress: null,
    p_certificates_earned: null,
    p_total_learning_hours: null,
    p_average_score: null,
    p_current_streak_days: null,
    p_longest_streak_days: null,
    p_xp_points: null,
    p_level: null,
    p_subscription_plan: null,
    p_subscription_start_date: null,
    p_subscription_end_date: null,
    p_total_amount_paid: null,
    p_has_active_subscription: null,
    p_referred_by_user_id: null,
    p_referral_code: null,
    p_is_seeking_job: null,
    p_preferred_job_roles: null,
    p_preferred_locations: null,
    p_expected_salary_range: null,
    p_resume_url: resumeUrl,
    p_portfolio_url: null,
    p_is_open_to_internship: null,
    p_is_open_to_freelance: null,
    p_is_active: null,
    p_actor_id: callerId,
  });
};
...
export const processStudentResumeUpload = async (
  id: number,
  file: Express.Multer.File,
  callerId: number | null
): Promise<void> => {
  // 1. Student profile must exist before we burn a Bunny round-trip on it.
  const existing = await getStudentProfileById(id);
  if (!existing) {
  ...
  const targetPath = `student-profiles/${id}/resume-${timestamp}.${ext}`;

  // 4. Delete prior URL path (best-effort)
  const priorPath = extractBunnyPath(existing.resumeUrl);
  if (priorPath) {
  ...
  await setStudentProfileResume(id, cdnUrl, callerId);
};
