...
// ─── Photo uploads (Bunny CDN) ────────────────────────────────────
//
// Pipeline: multer buffer → sharp validate/resize → sharp re-encode
// (WebP) → delete prior image(s) from Bunny → bunnyStorageService.upload(...)
// → udf_update_user_profiles with the resulting CDN URL.
//
// Key rules baked in here (NOT in the route) so the contract survives
// any future caller:
//   • max dimensions: 1024×1024 px, fit:inside, no enlargement
//   • output is always WebP
//   • storage keys are deterministic (`user-profiles/{userId}/profile.webp`)
 ...
 * files we own — if a photo URL is pointing at an external URL we
...
const safeDeleteFromBunny = async (
  ...
  try {
    ...
    logger.warn(
      { err, path, profileId },
      'User profile photo: pre-upload delete failed (object may not exist); continuing with new upload'
    );
  }
};

/**
 * Internal-only: persist new photo URLs after successful Bunny uploads.
 * All fields except the photo URLs are passed as null.
 */
const setUserProfilePhotos = async (
  id: number,
  profilePhotoUrl: string | null,
  coverPhotoUrl: string | null,
  callerId: number | null
): Promise<void> => {
  await db.callFunction('udf_update_user_profiles', {
    p_id: id,
    ...
    p_city_id: null,
    p_pincode: null,
    p_current_address_line_1: null,
    p_current_address_line_2: null,
    p_current_landmark: null,
    p_current_country_id: null,
    p_current_state_id: null,
    p_current_city_id: null,
    p_current_pincode: null,
    p_is_same_as_permanent: null,
    p_alternate_email: null,
    p_alternate_mobile: null,
    p_whatsapp_number: null,
    p_emergency_contact_name: null,
    p_emergency_contact_phone: null,
    p_emergency_contact_relation: null,
    p_aadhar_number: null,
    p_pan_number: null,
    p_passport_number: null,
    p_bank_name: null,
    p_bank_account_number: null,
    p_bank_ifsc_code: null,
    p_bank_branch: null,
    p_bank_account_type: null,
    p_upi_id: null,
    p_gst_number: null,
    p_preferred_language_id: null,
    p_timezone: null,
    p_theme_preference: null,
    p_email_notifications: null,
    p_sms_notifications: null,
    p_push_notifications: null,
    p_profile_completion: null,
    p_is_profile_complete: null,
    p_actor_id: callerId
  });
};

export const processUserProfilePhotoUpload = async (
  id: number,
  files: { profilePhoto?: Express.Multer.File; coverPhoto?: Express.Multer.File },
  callerId: number | null
): Promise<void> => {
  // 1. Profile must exist before we burn a Bunny round-trip on it.
  const existing = await getUserProfileById(id);
  if (!existing) {
    throw AppError.notFound(`User profile ${id} not found`);
  }

  // 2. Process profilePhoto if present: decode, resize, encode to WebP
  let newProfilePhotoUrl: string | null = existing.profilePhotoUrl;
  if (files.profilePhoto) {
    let metadata: sharp.Metadata;
    try {
      metadata = await sharp(files.profilePhoto.buffer).metadata();
    } catch (err) {
      logger.warn({ err, profileId: id }, 'User profile photo: sharp failed to read metadata');
      throw AppError.badRequest('Uploaded file is not a readable image');
    }

    let webpBuffer: Buffer;
    try {
      webpBuffer = await sharp(files.profilePhoto.buffer)
        .resize({ width: 1024, height: 1024, fit: 'inside', withoutEnlargement: true })
        .webp({ quality: 85 })
        .toBuffer();
    } catch (err) {
      logger.error({ err, profileId: id }, 'User profile photo: sharp WebP encode failed');
      throw AppError.internal('Failed to convert profile photo to WebP');
    }

    const targetPath = `user-profiles/${existing.userId}/profile.webp`;
    const priorPath = extractBunnyPath(existing.profilePhotoUrl);

    // Delete prior object before uploading new one (best-effort)
    if (priorPath) {
    ...
    newProfilePhotoUrl = cdnUrl;
  }

  // 3. Process coverPhoto if present: decode, resize, encode to WebP
  let newCoverPhotoUrl: string | null = existing.coverPhotoUrl;
  if (files.coverPhoto) {
    let metadata: sharp.Metadata;
    try {
      metadata = await sharp(files.coverPhoto.buffer).metadata();
    } catch (err) {
      logger.warn({ err, profileId: id }, 'User profile cover photo: sharp failed to read metadata');
      throw AppError.badRequest('Uploaded file is not a readable image');
    }

    let webpBuffer: Buffer;
    try {
      webpBuffer = await sharp(files.coverPhoto.buffer)
        .resize({ width: 1024, height: 1024, fit: 'inside', withoutEnlargement: true })
        .webp({ quality: 85 })
        .toBuffer();
    } catch (err) {
      logger.error({ err, profileId: id }, 'User profile cover photo: sharp WebP encode failed');
      throw AppError.internal('Failed to convert cover photo to WebP');
    }

    const targetPath = `user-profiles/${existing.userId}/cover.webp`;
    const priorPath = extractBunnyPath(existing.coverPhotoUrl);
    ...
    newCoverPhotoUrl = cdnUrl;
  }

  // 4. Persist the URLs via the internal-only setter.
  await setUserProfilePhotos(id, newProfilePhotoUrl, newCoverPhotoUrl, callerId);
};
