...
// ─── Image upload pipeline (parent + translations) ─────────────
//
// Same Bunny WebP contract as phase-08 subjects — sharp re-encode with
// quality loop until ≤ IMAGE_MAX_BYTES (100 KB). Deterministic paths:
//   • parent trailer thumbnail → courses/<courseId>/trailer-thumbnail.webp
//   • translation slot URLs    → courses/translations/<translationId>/<segment>.webp
// Videos (trailerVideoUrl, videoUrl) and PDFs (brochureUrl) remain
// plain text URL columns — callers manage their own CDN upload.

export type CourseImageSlot = 'trailerThumbnail';
export type CourseImageFiles = Partial<Record<CourseImageSlot, Express.Multer.File>>;

export type CourseTranslationImageSlot =
  | 'webThumbnail'
  | 'webBanner'
  | 'appThumbnail'
  | 'appBanner'
  ...
  | 'ogImage'
  | 'twitterImage';
export type CourseTranslationImageFiles = Partial<
  Record<CourseTranslationImageSlot, Express.Multer.File>
>;

const COURSE_TRANSLATION_SLOT_CONFIG: Record<
  CourseTranslationImageSlot,
  { pathSegment: string; boxPx: number }
> = {
  webThumbnail: { pathSegment: 'web-thumbnail', boxPx: IMAGE_BOX_PX },
  webBanner: { pathSegment: 'web-banner', boxPx: IMAGE_BOX_PX },
  appThumbnail: { pathSegment: 'app-thumbnail', boxPx: IMAGE_BOX_PX },
  appBanner: { pathSegment: 'app-banner', boxPx: IMAGE_BOX_PX },
  videoThumbnail: { pathSegment: 'video-thumbnail', boxPx: IMAGE_BOX_PX },
  ogImage: { pathSegment: 'og-image', boxPx: IMAGE_BOX_PX },
  twitterImage: { pathSegment: 'twitter-image', boxPx: IMAGE_BOX_PX }
};

/** Patch a single trailer-thumbnail URL on the parent course row. */
const setCourseTrailerThumbnailUrl = async (
  id: number,
  url: string | null,
  callerId: number | null
): Promise<void> => {
  await db.callFunction('udf_update_courses', {
    ...
    p_course_language_id: null,
    p_is_instructor_course: null,
    p_code: null,
    p_slug: null,
    p_difficulty_level: null,
    p_course_status: null,
    ...
    p_currency: null,
    p_is_free: null,
    p_trailer_video_url: null,
    p_trailer_thumbnail_url: url,
    p_video_url: null,
    p_brochure_url: null,
    p_is_new: null,
    p_new_until: null,
    p_is_featured: null,
    p_is_bestseller: null,
    p_has_placement_assistance: null,
    p_has_certificate: null,
    p_max_students: null,
    p_refund_days: null,
    p_is_active: null,
    p_published_at: null,
    p_content_updated_at: null,
    p_actor_id: callerId
  });
};

/** Re-encode + upload a single trailer thumbnail and persist the URL. */
export const processCourseImageUploads = async (
  courseId: number,
  files: CourseImageFiles,
  callerId: number | null
): Promise<void> => {
  const file = files.trailerThumbnail;
  if (!file) return;
  const existing = await getCourseById(courseId);
  if (!existing) {
    throw AppError.notFound(`Course ${courseId} not found`);
  }
  // Read current url from the parent row via a thin query — getCourseById
  // returns the listing-flavored DTO without trailerThumbnailUrl, so we
  // need a small helper read.
  const cur = await db.query<{ course_trailer_thumbnail_url: string | null }>(
    'SELECT course_trailer_thumbnail_url FROM uv_courses WHERE course_id = $1 LIMIT 1',
    [courseId]
  );
  const currentUrl = cur.rows[0]?.course_trailer_thumbnail_url ?? null;

  const result = await replaceImage({
    inputBuffer: file.buffer,
    targetPath: `courses/${courseId}/trailer-thumbnail.webp`,
    currentUrl,
    boxPx: IMAGE_BOX_PX,
    maxBytes: IMAGE_MAX_BYTES,
    logContext: { courseId, slot: 'trailerThumbnail' }
  });
  if (!result) {
    throw AppError.badRequest(
      `Course trailer thumbnail is too complex to compress under ${Math.round(IMAGE_MAX_BYTES / 1024)} KB. Try a simpler image.`,
      { slot: 'trailerThumbnail', maxBytes: IMAGE_MAX_BYTES }
    );
  }
  await setCourseTrailerThumbnailUrl(courseId, result.cdnUrl, callerId);
};

/** Patch a single image-URL column on a course translation. */
const setCourseTranslationImageUrl = async (
  id: number,
  slot: CourseTranslationImageSlot,
  url: string | null,
  callerId: number | null
): Promise<void> => {
  await db.callFunction('udf_update_course_translations', {
    ...
    p_long_intro: null,
    p_tagline: null,
    p_web_thumbnail: slot === 'webThumbnail' ? url : null,
    p_web_banner: slot === 'webBanner' ? url : null,
    p_app_thumbnail: slot === 'appThumbnail' ? url : null,
    p_app_banner: slot === 'appBanner' ? url : null,
    p_video_title: null,
    p_video_description: null,
    p_video_thumbnail: slot === 'videoThumbnail' ? url : null,
    p_video_duration_minutes: null,
    p_tags: null,
    p_is_new_title: null,
    p_prerequisites: null,
    p_skills_gain: null,
    p_what_you_will_learn: null,
    p_course_includes: null,
    p_course_is_for: null,
    p_apply_for_designations: null,
    p_demand_in_countries: null,
    p_salary_standard: null,
    p_future_courses: null,
    p_meta_title: null,
...
/** Re-encode each translation slot file to WebP, upload to Bunny, persist URLs. */
export const processCourseTranslationImageUploads = async (
  translationId: number,
  files: CourseTranslationImageFiles,
  callerId: number | null
): Promise<void> => {
  const existing = await getCourseTranslationById(translationId);
  if (!existing) {
    throw AppError.notFound(`Course translation ${translationId} not found`);
  }
  const currentUrls: Record<CourseTranslationImageSlot, string | null> = {
    webThumbnail: existing.webThumbnail,
    webBanner: existing.webBanner,
    appThumbnail: existing.appThumbnail,
    appBanner: existing.appBanner,
    videoThumbnail: existing.videoThumbnail,
    ogImage: existing.ogImage,
    twitterImage: existing.twitterImage
  };

  const slots: CourseTranslationImageSlot[] = [
    'webThumbnail',
    'webBanner',
    'appThumbnail',
    'appBanner',
  ...
  for (const slot of slots) {
    ...
    if (!file) continue;
    const cfg = COURSE_TRANSLATION_SLOT_CONFIG[slot];
    const result = await replaceImage({
      inputBuffer: file.buffer,
      targetPath: `courses/translations/${translationId}/${cfg.pathSegment}.webp`,
      currentUrl: currentUrls[slot],
      boxPx: cfg.boxPx,
      maxBytes: IMAGE_MAX_BYTES,
      logContext: { courseTranslationId: translationId, slot }
    });
    if (!result) {
      throw AppError.badRequest(
        `Course translation ${slot} is too complex to compress under ${Math.round(IMAGE_MAX_BYTES / 1024)} KB. Try a simpler image.`,
        { slot, maxBytes: IMAGE_MAX_BYTES }
      );
    }
    await setCourseTranslationImageUrl(translationId, slot, result.cdnUrl, callerId);
  }
};
