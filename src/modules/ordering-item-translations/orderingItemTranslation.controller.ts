...
export async function list(req: Request, res: Response) {
  ...
  if (search) q = applySearch(q, search, { ilike: ['item_text'] });
...
export async function softDelete(req: Request, res: Response) {
  const id = parseInt(req.params.id);
  const { data: old } = await supabase.from('ordering_item_translations').select('ordering_item_id, deleted_at').eq('id', id).single();
  if (!old) return err(res, 'Ordering item translation not found', 404);
...
export async function coverage(req: Request, res: Response) {
  ...
  let q = supabase.from('ordering_item_translations').select('language_id, languages(name, native_name, iso_code)').is('deleted_at', null);
