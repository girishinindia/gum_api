...
const TABLE = 'coupon_bundles';
const CACHE_KEY = 'coupon_bundles:all';
const clearCache = async () => { await redis.del(CACHE_KEY); };

function parseBody(req: Request): any {
  const body: any = { ...req.body };
  if (typeof body.is_active === 'string') body.is_active = body.is_active === 'true';
  for (const k of ['coupon_id', 'bundle_id']) {
...
const FK_SELECT = `*, coupons(id, coupon_code, title), bundles(id, name)`;
...
export async function getById(req: Request, res: Response) {
  ...
  if (e || !data) return err(res, 'Coupon-bundle link not found', 404);
  return ok(res, data);
}
...
export async function create(req: Request, res: Response) {
  ...
  if (e) return err(res, e.message, 500);
  await clearCache();
  logAdmin({ actorId: req.user!.id, action: 'coupon_bundle_created', targetType: 'coupon_bundle', targetId: data.id, targetName: `Coupon-Bundle #${data.id}`, ip: getClientIp(req) });
  return ok(res, data, 'Coupon-bundle link created', 201);
}

export async function update(req: Request, res: Response) {
  const id = parseInt(req.params.id);
  const { data: old } = await supabase.from(TABLE).select('*').eq('id', id).single();
  if (!old) return err(res, 'Coupon-bundle link not found', 404);
  const updates = parseBody(req);
  updates.updated_by = req.user!.id;
  const { data, error: e } = await supabase.from(TABLE).update(updates).eq('id', id).select(FK_SELECT).single();
  if (e) return err(res, e.message, 500);
  await clearCache();
  logAdmin({ actorId: req.user!.id, action: 'coupon_bundle_updated', targetType: 'coupon_bundle', targetId: id, targetName: `Coupon-Bundle #${id}`, ip: getClientIp(req) });
  return ok(res, data, 'Coupon-bundle link updated');
}
...
export async function softDelete(req: Request, res: Response) {
  ...
  if (e) return err(res, e.message, 500);
  await clearCache();
  logAdmin({ actorId: req.user!.id, action: 'coupon_bundle_soft_deleted', targetType: 'coupon_bundle', targetId: id, targetName: `Coupon-Bundle #${id}`, ip: getClientIp(req) });
  return ok(res, data, 'Coupon-bundle link moved to trash');
}
...
export async function restore(req: Request, res: Response) {
  ...
  if (e) return err(res, e.message, 500);
  await clearCache();
  logAdmin({ actorId: req.user!.id, action: 'coupon_bundle_restored', targetType: 'coupon_bundle', targetId: id, targetName: `Coupon-Bundle #${id}`, ip: getClientIp(req) });
  return ok(res, data, 'Coupon-bundle link restored');
}
...
export async function remove(req: Request, res: Response) {
  ...
  if (e) return err(res, e.message, 500);
  await clearCache();
  logAdmin({ actorId: req.user!.id, action: 'coupon_bundle_deleted', targetType: 'coupon_bundle', targetId: id, targetName: `Coupon-Bundle #${id}`, ip: getClientIp(req) });
  return ok(res, null, 'Coupon-bundle link permanently deleted');
}
