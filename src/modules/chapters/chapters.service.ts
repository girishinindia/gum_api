...
export type ChapterTranslationImageSlot = 'icon' | 'image' | 'ogImage' | 'twitterImage';
export type ChapterTranslationImageFiles = Partial<
  Record<ChapterTranslationImageSlot, Express.Multer.File>
>;

const CHAPTER_TRANSLATION_SLOT_CONFIG: Record<
  ChapterTranslationImageSlot,
...
const setChapterTranslationImageUrl = async (
  id: number,
  slot: ChapterTranslationImageSlot,
  url: string | null,
  callerId: number | null
): Promise<void> => {
  await db.callFunction('udf_update_chapter_translations', {
...
export const processChapterTranslationImageUploads = async (
  translationId: number,
  files: ChapterTranslationImageFiles,
  callerId: number | null
): Promise<void> => {
  const existing = await getChapterTranslationById(translationId);
  if (!existing) {
    throw AppError.notFound(`Chapter translation ${translationId} not found`);
  }

  const currentUrls: Record<ChapterTranslationImageSlot, string | null> = {
    icon: existing.icon,
  ...
  const slots: ChapterTranslationImageSlot[] = ['icon', 'image', 'ogImage', 'twitterImage'];
  for (const slot of slots) {
    const file = files[slot];
    if (!file) continue;
    const cfg = CHAPTER_TRANSLATION_SLOT_CONFIG[slot];
    const result = await replaceImage({
      inputBuffer: file.buffer,
      targetPath: `chapters/translations/${translationId}/${cfg.pathSegment}.webp`,
      currentUrl: currentUrls[slot],
      boxPx: cfg.boxPx,
      maxBytes: IMAGE_MAX_BYTES,
      logContext: { chapterTranslationId: translationId, slot }
    });
    if (!result) {
      throw AppError.badRequest(
        `Chapter translation ${slot} is too complex to compress under ${Math.round(IMAGE_MAX_BYTES / 1024)} KB. Try a simpler image.`,
        { slot, maxBytes: IMAGE_MAX_BYTES }
      );
    }
    await setChapterTranslationImageUrl(translationId, slot, result.cdnUrl, callerId);
  }
};
