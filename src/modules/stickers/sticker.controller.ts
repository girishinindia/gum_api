...
export async function getById(req: Request, res: Response) {
  ...
  if (e || !data) return err(res, 'Sticker not found', 404);
  return ok(res, data);
}
...
export async function create(req: Request, res: Response) {
  ...
  clearCache();
  logAdmin({ actorId: req.user!.id, action: 'sticker_created', targetType: 'sticker', targetId: data.id, targetName: body.name, ip: getClientIp(req) });
  return ok(res, data, 'Sticker created', 201);
}
...
export async function update(req: Request, res: Response) {
  ...
  clearCache();
  logAdmin({ actorId: req.user!.id, action: 'sticker_updated', targetType: 'sticker', targetId: id, targetName: body.name || old.name, ip: getClientIp(req) });
  return ok(res, data, 'Sticker updated');
}

// ── DELETE /stickers/:id (soft) ──
export async function softDelete(req: Request, res: Response) {
  ...
  clearCache();
  logAdmin({ actorId: req.user!.id, action: 'sticker_soft_deleted', targetType: 'sticker', targetId: id, targetName: old.name, ip: getClientIp(req) });
  return ok(res, data, 'Sticker moved to trash');
}

// ── PATCH /stickers/:id/restore ──
export async function restore(req: Request, res: Response) {
  ...
  clearCache();
  logAdmin({ actorId: req.user!.id, action: 'sticker_restored', targetType: 'sticker', targetId: id, targetName: old.name, ip: getClientIp(req) });
  return ok(res, data, 'Sticker restored');
}
...
export async function remove(req: Request, res: Response) {
  ...
  clearCache();
  logAdmin({ actorId: req.user!.id, action: 'sticker_deleted', targetType: 'sticker', targetId: id, targetName: old.name, ip: getClientIp(req) });
  return ok(res, null, 'Sticker permanently deleted');
}
