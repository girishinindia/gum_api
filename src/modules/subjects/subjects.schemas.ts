...
// twitterImage), so an empty text body is legitimate when the caller is
// only replacing one or more image files. The route handler enforces the
// combined `hasTextChange || hasFile` guard and throws 400 if both are missing.
