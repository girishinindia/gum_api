...
const TABLE = 'coupon_batches';
const CACHE_KEY = 'coupon_batches:all';
const clearCache = async () => { await redis.del(CACHE_KEY); };

function parseBody(req: Request): any {
  const body: any = { ...req.body };
  if (typeof body.is_active === 'string') body.is_active = body.is_active === 'true';
  for (const k of ['coupon_id', 'batch_id']) {
...
const FK_SELECT = `*, coupons(id, coupon_code, title), course_batches(id, title)`;
...
export async function getById(req: Request, res: Response) {
  ...
  if (e || !data) return err(res, 'Coupon-batch link not found', 404);
  return ok(res, data);
}
...
export async function create(req: Request, res: Response) {
  ...
  if (!body.batch_id) return err(res, 'batch_id is required', 400);
  ...
  if (coupon.applicable_to !== 'all' && coupon.applicable_to !== 'batch') {
    return err(res, `This coupon applies to '${coupon.applicable_to}', so it can't be linked to a batch`, 400);
  }
  ...
  if (e) return err(res, e.message, 500);
  await clearCache();
  logAdmin({ actorId: req.user!.id, action: 'coupon_batch_created', targetType: 'coupon_batch', targetId: data.id, targetName: `Coupon-Batch #${data.id}`, ip: getClientIp(req) });
  return ok(res, data, 'Coupon-batch link created', 201);
}

export async function update(req: Request, res: Response) {
  const id = parseInt(req.params.id);
  const { data: old } = await supabase.from(TABLE).select('*').eq('id', id).single();
  if (!old) return err(res, 'Coupon-batch link not found', 404);
  const updates = parseBody(req);
  updates.updated_by = req.user!.id;
  const { data, error: e } = await supabase.from(TABLE).update(updates).eq('id', id).select(FK_SELECT).single();
  if (e) return err(res, e.message, 500);
  await clearCache();
  logAdmin({ actorId: req.user!.id, action: 'coupon_batch_updated', targetType: 'coupon_batch', targetId: id, targetName: `Coupon-Batch #${id}`, ip: getClientIp(req) });
  return ok(res, data, 'Coupon-batch link updated');
}
...
export async function softDelete(req: Request, res: Response) {
  ...
  if (e) return err(res, e.message, 500);
  await clearCache();
  logAdmin({ actorId: req.user!.id, action: 'coupon_batch_soft_deleted', targetType: 'coupon_batch', targetId: id, targetName: `Coupon-Batch #${id}`, ip: getClientIp(req) });
  return ok(res, data, 'Coupon-batch link moved to trash');
}
...
export async function restore(req: Request, res: Response) {
  ...
  if (e) return err(res, e.message, 500);
  await clearCache();
  logAdmin({ actorId: req.user!.id, action: 'coupon_batch_restored', targetType: 'coupon_batch', targetId: id, targetName: `Coupon-Batch #${id}`, ip: getClientIp(req) });
  return ok(res, data, 'Coupon-batch link restored');
}
...
export async function remove(req: Request, res: Response) {
  ...
  if (e) return err(res, e.message, 500);
  await clearCache();
  logAdmin({ actorId: req.user!.id, action: 'coupon_batch_deleted', targetType: 'coupon_batch', targetId: id, targetName: `Coupon-Batch #${id}`, ip: getClientIp(req) });
  return ok(res, null, 'Coupon-batch link permanently deleted');
}
