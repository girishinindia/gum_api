...
function parseBody(req: Request): any {
  ...
  for (const k of ['parent_id', 'display_order']) {
...
export async function getById(req: Request, res: Response) {
  ...
  if (e || !data) return err(res, 'Blog category not found', 404);
  return ok(res, data);
}
...
export async function create(req: Request, res: Response) {
  ...
  return ok(res, data, 'Blog category created', 201);
}
...
export async function update(req: Request, res: Response) {
  ...
  if (!old) return err(res, 'Blog category not found', 404);
  ...
  return ok(res, data, 'Blog category updated');
}
...
export async function softDelete(req: Request, res: Response) {
  ...
  await clearCache();
  logAdmin({ actorId: req.user!.id, action: 'blog_category_soft_deleted', targetType: 'blog_category', targetId: id, targetName: old.name, ip: getClientIp(req) });
  return ok(res, data, 'Blog category moved to trash');
}
...
export async function restore(req: Request, res: Response) {
  ...
  await clearCache();
  logAdmin({ actorId: req.user!.id, action: 'blog_category_restored', targetType: 'blog_category', targetId: id, targetName: old.name, ip: getClientIp(req) });
  return ok(res, data, 'Blog category restored');
}
...
export async function remove(req: Request, res: Response) {
  ...
  await clearCache();
  logAdmin({ actorId: req.user!.id, action: 'blog_category_deleted', targetType: 'blog_category', targetId: id, targetName: old.name, ip: getClientIp(req) });
  return ok(res, null, 'Blog category permanently deleted');
}
