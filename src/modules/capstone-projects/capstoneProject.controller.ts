...
const TABLE = 'assesment_capstone_projects';
const TRANS_TABLE = 'assesment_capstone_projects_translations';
const SOLUTION_TABLE = 'assesment_capstone_projects_solution';
const CACHE_KEY = 'assesment_capstone_projects:all';

const clearCache = async () => {
  await redis.del(CACHE_KEY);
};

const FK_SELECT = `*, courses!assesment_capstone_projects_course_id_fkey(name, slug), ${TRANS_TABLE}(id, language_id, name)`;

function parseMultipartBody(req: Request): any {
  const body: any = { ...req.body };
  if (typeof body.is_active === 'string') body.is_active = body.is_active === 'true';
  if (typeof body.course_id === 'string') {
    body.course_id = body.course_id === '' || body.course_id === 'null' ? null : parseInt(body.course_id) || null;
  }
...
/**
 * Build CDN path for capstone project solution ZIP.
 * Format: materials/<course-slug>/capstone/solutions/<slug>.zip
 */
async function buildSolutionCdnPath(courseId: number, slug: string): Promise<string | null> {
  ...
  return `materials/${course.slug}/capstone/solutions/${slug}_solution.zip`;
}

/**
 * Build CDN path for capstone project translation HTML.
 * Format: materials/<course-slug>/capstone/<lang-iso>/<slug>.html
 */
async function buildTranslationCdnPath(courseId: number, langIsoCode: string, slug: string): Promise<string | null> {
  const { data: course } = await supabase
    .from('courses')
    .select('slug')
    .eq('id', courseId)
    .single();
  if (!course?.slug) return null;
  return `materials/${course.slug}/capstone/${langIsoCode}/${slug}.html`;
}
...
export async function create(req: Request, res: Response) {
  ...
  const slugSource = body.name || body.slug_source || 'capstone-project';
  body.slug = await generateUniqueSlug(supabase, TABLE, slugSource, undefined, { column: 'course_id', value: body.course_id });
  delete body.name;
  delete body.slug_source;

  // Handle solution ZIP upload
  if (files?.file_solution?.[0] && body.course_id) {
    const cdnPath = await buildSolutionCdnPath(body.course_id, body.slug);
    if (cdnPath) {
  ...
  const { data, error: e } = await supabase
    ...
    .single();
  if (e) {
    if (e.code === '23505') return err(res, 'Capstone project slug already exists for this course', 409);
    return err(res, e.message, 500);
  }

  await clearCache();
  logAdmin({ actorId: req.user!.id, action: 'capstone_project_created', targetType: 'capstone_project', targetId: data.id, targetName: data.slug, ip: getClientIp(req) });
...
export async function update(req: Request, res: Response) {
  ...
  if (updates.name) {
    updates.slug = await generateUniqueSlug(supabase, TABLE, updates.name, id, { column: 'course_id', value: updates.course_id || old.course_id });
    delete updates.name;
  }
  ...
  if (files?.file_solution?.[0]) {
    ...
    const courseId = updates.course_id || old.course_id;
    const slug = updates.slug || old.slug;
    const cdnPath = await buildSolutionCdnPath(courseId, slug);
    if (cdnPath) {
      const cdnUrl = await uploadToBunny(cdnPath, files.file_solution[0].buffer);
...
export async function softDelete(req: Request, res: Response) {
  ...
  await supabase.from(TRANS_TABLE).update({ deleted_at: now, is_active: false }).eq('capstone_project_id', id).is('deleted_at', null);
...
export async function restore(req: Request, res: Response) {
  ...
  // Cascade restore
  await supabase.from(TRANS_TABLE).update({ deleted_at: null, is_active: true }).eq('capstone_project_id', id).not('deleted_at', 'is', null);
...
export async function remove(req: Request, res: Response) {
  ...
  const { data: translations } = await supabase.from(TRANS_TABLE).select('id, file_url').eq('capstone_project_id', id);
  if (translations) {
  ...
  await supabase.from(SOLUTION_TABLE).delete().eq('capstone_project_id', id);
  await supabase.from(TRANS_TABLE).delete().eq('capstone_project_id', id);
...
/* --- Full endpoints (capstone project + English translation) --- */
...
export async function getFullById(req: Request, res: Response) {
  ...
  const { data: project, error: e } = await supabase
    ...
    .eq('id', id)
    .single();
  if (e || !project) return err(res, 'Capstone project not found', 404);
...
export async function createFull(req: Request, res: Response) {
  ...
  const slugSource = body.name || body.title || 'capstone-project';
  projectData.slug = await generateUniqueSlug(supabase, TABLE, slugSource, undefined, { column: 'course_id', value: body.course_id });
  projectData.created_by = req.user!.id;

  // Handle solution ZIP upload
  if (files?.file_solution?.[0] && body.course_id) {
    const cdnPath = await buildSolutionCdnPath(body.course_id, projectData.slug);
    if (cdnPath) {
      const cdnUrl = await uploadToBunny(cdnPath, files.file_solution[0].buffer);
  ...
  const { data: project, error: exErr } = await supabase
    ...
    .single();
  if (exErr) {
    if (exErr.code === '23505') return err(res, 'Capstone project slug already exists for this course', 409);
  ...
  if (files?.file?.[0] && body.course_id) {
    const cdnPath = await buildTranslationCdnPath(body.course_id, 'en', project.slug);
    if (cdnPath) {
  ...
  await clearCache();
  logAdmin({ actorId: req.user!.id, action: 'capstone_project_created_full', targetType: 'capstone_project', targetId: project.id, targetName: project.slug, ip: getClientIp(req) });
  return ok(res, { ...project, [TRANS_TABLE]: [translation] }, 'Capstone project created with English translation', 201);
}
...
export async function updateFull(req: Request, res: Response) {
  ...
  if (body.name) {
    projectUpdates.slug = await generateUniqueSlug(supabase, TABLE, body.name, id, { column: 'course_id', value: body.course_id || old.course_id });
  }
  ...
  if (files?.file_solution?.[0]) {
    ...
    const courseId = body.course_id || old.course_id;
    const slug = projectUpdates.slug || old.slug;
    const cdnPath = await buildSolutionCdnPath(courseId, slug);
    if (cdnPath) {
      const cdnUrl = await uploadToBunny(cdnPath, files.file_solution[0].buffer);
  ...
  if (files?.file?.[0]) {
    ...
    const courseId = body.course_id || old.course_id;
    const slug = projectUpdates.slug || old.slug;
    const cdnPath = await buildTranslationCdnPath(courseId, 'en', slug);
    if (cdnPath) {
      const cdnUrl = await uploadToBunny(cdnPath, files.file[0].buffer);
  ...
  if (existingTrans) {
    ...
    const { data: t } = await supabase
      .from(TRANS_TABLE)
      .insert({ capstone_project_id: id, language_id: 7, name: body.name || old.slug, ...translationUpdates, created_by: req.user!.id })
      .select('*, languages(id, name, iso_code, native_name)')
      .single();
    translation = t;
  }
