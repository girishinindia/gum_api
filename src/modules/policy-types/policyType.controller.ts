...
const CACHE_KEY = 'policy_types:all';
...
export async function list(req: Request, res: Response) {
  ...
  if (search) q = q.or(`name.ilike.%${search}%,description.ilike.%${search}%,code.ilike.%${search}%`);
...
export async function getById(req: Request, res: Response) {
  ...
  if (e || !data) return err(res, 'Policy type not found', 404);
  return ok(res, data);
}
...
export async function create(req: Request, res: Response) {
  ...
  await clearCache();
  logAdmin({ actorId: req.user!.id, action: 'policy_type_created', targetType: 'policy_type', targetId: data.id, targetName: body.name, ip: getClientIp(req) });
  return ok(res, data, 'Policy type created', 201);
}
...
export async function update(req: Request, res: Response) {
  ...
  await clearCache();
  logAdmin({ actorId: req.user!.id, action: 'policy_type_updated', targetType: 'policy_type', targetId: id, targetName: updates.name || old.name, ip: getClientIp(req) });
  return ok(res, data, 'Policy type updated');
}
...
export async function restore(req: Request, res: Response) {
  ...
  return ok(res, data, 'Policy type restored');
}
...
export async function remove(req: Request, res: Response) {
  ...
  await clearCache();
  logAdmin({ actorId: req.user!.id, action: 'policy_type_deleted', targetType: 'policy_type', targetId: id, targetName: old.name, ip: getClientIp(req) });
  return ok(res, null, 'Policy type permanently deleted');
}
