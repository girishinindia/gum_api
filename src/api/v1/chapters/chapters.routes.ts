...
import type { ChapterTranslationImageFiles } from '../../../modules/chapters/chapters.service';

const collectChapterTranslationFiles = (req: Parameters<typeof getSlotFile>[0]): ChapterTranslationImageFiles => {
  const files: ChapterTranslationImageFiles = {};
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const files = collectChapterTranslationFiles(req);
    ...
    if (Object.keys(files).length > 0) {
      await chaptersService.processChapterTranslationImageUploads(
        result.id,
        files,
        req.user?.id ?? null
      );
