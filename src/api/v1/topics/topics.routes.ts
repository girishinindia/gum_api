...
import type { TopicTranslationImageFiles } from '../../../modules/topics/topics.service';

const collectTopicTranslationFiles = (req: Parameters<typeof getSlotFile>[0]): TopicTranslationImageFiles => {
  const files: TopicTranslationImageFiles = {};
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const files = collectTopicTranslationFiles(req);
    ...
    if (Object.keys(files).length > 0) {
      await topicsService.processTopicTranslationImageUploads(
        result.id,
        files,
        req.user?.id ?? null
      );
