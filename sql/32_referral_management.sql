...
--  referral_rewards, and updates admin_activity_log_action_check.
-- ═══════════════════════════════════════════════════════════════

-- ─── Permissions: referral_code ───
INSERT INTO permissions (resource, action, display_name, description) VALUES
  ('referral_code', 'create', 'Create Referral Code', 'Create referral code records'),
  ('referral_code', 'read', 'View Referral Codes', 'View referral code records'),
  ('referral_code', 'update', 'Update Referral Code', 'Update referral code records'),
  ('referral_code', 'delete', 'Delete Referral Code', 'Permanently delete referral code records'),
  ('referral_code', 'soft_delete', 'Soft Delete Referral Code', 'Soft delete referral code records'),
  ('referral_code', 'restore', 'Restore Referral Code', 'Restore soft-deleted referral code records')
ON CONFLICT (resource, action) DO NOTHING;

-- ─── Permissions: referral_usage ───
INSERT INTO permissions (resource, action, display_name, description) VALUES
  ('referral_usage', 'create', 'Create Referral Usage', 'Create referral usage records'),
  ('referral_usage', 'read', 'View Referral Usages', 'View referral usage records'),
  ('referral_usage', 'update', 'Update Referral Usage', 'Update referral usage records'),
  ('referral_usage', 'delete', 'Delete Referral Usage', 'Permanently delete referral usage records'),
  ('referral_usage', 'soft_delete', 'Soft Delete Referral Usage', 'Soft delete referral usage records'),
  ('referral_usage', 'restore', 'Restore Referral Usage', 'Restore soft-deleted referral usage records')
ON CONFLICT (resource, action) DO NOTHING;

-- ─── Permissions: referral_reward ───
INSERT INTO permissions (resource, action, display_name, description) VALUES
  ('referral_reward', 'create', 'Create Referral Reward', 'Create referral reward records'),
  ('referral_reward', 'read', 'View Referral Rewards', 'View referral reward records'),
  ('referral_reward', 'update', 'Update Referral Reward', 'Update referral reward records'),
  ('referral_reward', 'delete', 'Delete Referral Reward', 'Permanently delete referral reward records'),
  ('referral_reward', 'soft_delete', 'Soft Delete Referral Reward', 'Soft delete referral reward records'),
  ('referral_reward', 'restore', 'Restore Referral Reward', 'Restore soft-deleted referral reward records')
ON CONFLICT (resource, action) DO NOTHING;

-- ─── Grant all referral permissions to super_admin role ───
INSERT INTO role_permissions (role_id, permission_id)
SELECT r.id, p.id FROM roles r CROSS JOIN permissions p
WHERE r.name = 'super_admin' AND p.resource IN ('referral_code', 'referral_usage', 'referral_reward')
...
ALTER TABLE admin_activity_log ADD CONSTRAINT admin_activity_log_action_check CHECK (action::text = ANY(ARRAY[
  ...
  -- Webinars
  'webinar_created', 'webinar_updated', 'webinar_deleted', 'webinar_soft_deleted', 'webinar_restored',
  'webinar_translation_created', 'webinar_translation_updated', 'webinar_translation_deleted', 'webinar_translation_soft_deleted', 'webinar_translation_restored',
  -- Referral Codes
  'referral_code_created', 'referral_code_updated', 'referral_code_deleted', 'referral_code_soft_deleted', 'referral_code_restored',
  -- Referral Usages
  'referral_usage_created', 'referral_usage_updated', 'referral_usage_deleted', 'referral_usage_soft_deleted', 'referral_usage_restored',
  -- Referral Rewards
  'referral_reward_created', 'referral_reward_updated', 'referral_reward_deleted', 'referral_reward_soft_deleted', 'referral_reward_restored'
]::text[]));

-- ─── Table Summary ───
INSERT INTO table_summary (table_name, description) VALUES
  ('referral_codes', 'Referral codes assigned to students with discount percentages, reward settings, and usage limits'),
  ('referral_usages', 'Tracks when a referral code is used by a referred user with discount and status details'),
  ('referral_rewards', 'Tracks cashback/wallet credit rewards earned by referrers from successful referrals')
