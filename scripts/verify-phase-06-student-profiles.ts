...
/**
 * Stage 6 — Phase-06 student-profiles, live end-to-end verification.
 *
 ...
 *
 * Coverage (matches the explicit ask for phase 06):
 ...
 *   2. SA CRUD     — create / list / get / patch (core fields) on student
 ...
 *   6. SA delete   — DELETE /:id student profile → 200, verify 404.
 *   7. Cleanup     — hard-delete leftover student profiles via direct
...
const SA_EMAIL = `verify-sp-sa+${RUN_ID}@test.growupmore.local`;
const SA_PASSWORD = 'VerifyStudentProfiles123';
const ADMIN_EMAIL = `verify-sp-admin+${RUN_ID}@test.growupmore.local`;
const ADMIN_PASSWORD = 'VerifyStudentProfiles123';
const STUDENT_EMAIL = `verify-sp-student+${RUN_ID}@test.growupmore.local`;
const STUDENT_PASSWORD = 'VerifyStudentProfiles123';
...
// The student user's student profile is SA-created and used as the "another
...
const hardDeleteStudentProfile = async (id: number): Promise<void> => {
  await getPool().query('DELETE FROM student_profiles WHERE id = $1', [id]);
};

const hardDeleteStudentProfileByUserId = async (userId: number): Promise<void> => {
  await getPool().query('DELETE FROM student_profiles WHERE user_id = $1', [userId]);
};
...
interface ListResponse {
  data?: Array<{ id: number; userId: number; enrollmentNumber: string }>;
  meta?: { totalCount: number; page: number; limit: number; totalPages: number };
}

const main = async (): Promise<void> => {
  console.log('━━ Stage 6 · Student profiles verify (live) ━━');
  ...
  try {
    ...
    header('0. Setup — register + elevate + login (sa, admin, student)');
    {
      ...
      record(
        '0',
        'sa JWT carries student_profile.* perms (≥ 5 total)',
        saPermCount >= 5,
      ...
      record(
        '0',
        'admin JWT has student_profile.read and student_profile.update and NOT delete',
        adminPerms.includes('student_profile.read') &&
          adminPerms.includes('student_profile.update') &&
          !adminPerms.includes('student_profile.delete'),
      ...
      record(
        '0',
        'student JWT lacks global student_profile perms',
        !studentPerms.includes('student_profile.read') &&
          !studentPerms.includes('student_profile.create') &&
          !studentPerms.includes('student_profile.delete'),
    ...
    {
      const a = await http('GET', '/api/v1/student-profiles');
      record('1', 'GET /student-profiles (no token) → 401', a.status === 401, `got ${a.status}`);
      const b = await http('POST', '/api/v1/student-profiles', {
        body: { userId: 1, enrollmentNumber: 'TEST' }
      });
      record(
        '1',
        'POST /student-profiles (no token) → 401',
        b.status === 401,
        `got ${b.status}`
      );
      const c = await http('DELETE', '/api/v1/student-profiles/1');
      record(
        '1',
        'DELETE /student-profiles/1 (no token) → 401',
        c.status === 401,
        `got ${c.status}`
      );
    }
    ...
    header('2. Super Admin — create / list / get / patch (core fields)');
    {
      ...
      const create = await http<StudentProfileResponse>('POST', '/api/v1/student-profiles', {
        token: saToken,
        body: {
          userId: studentUserId,
          enrollmentNumber: `STU-TEST-${RUN_ID}`
        }
      });
      record(
        '2',
        'POST /student-profiles (sa, required fields) → 201',
      ...
      // 2.b SA list
      const list = await http<ListResponse>('GET', '/api/v1/student-profiles?pageSize=5', {
        token: saToken
      });
      record(
        '2',
        'GET /student-profiles?pageSize=5 (sa) → 200',
        list.status === 200,
        `status=${list.status}`
      );
      record(
      ...
      record(
        ...
        (list.body?.data ?? []).some((r) => r.id === studentProfileId),
        `ids=${(list.body?.data ?? []).map((r) => r.id).join(',')}`
      );

      // 2.c SA GET /:id
      const getOne = await http<StudentProfileResponse>('GET', `/api/v1/student-profiles/${studentProfileId}`, {
        token: saToken
      });
      record(
        '2',
        'GET /student-profiles/:id (sa) → 200',
        getOne.status === 200 &&
          getOne.body?.data?.id === studentProfileId &&
          getOne.body?.data?.userId === studentUserId,
        `status=${getOne.status} id=${getOne.body?.data?.id}`
      );
      record(
        '2',
        'profile has enrollmentNumber and nested user object',
        getOne.body?.data?.enrollmentNumber === `STU-TEST-${RUN_ID}` &&
          getOne.body?.data?.user?.email === STUDENT_EMAIL,
        `code=${getOne.body?.data?.enrollmentNumber} user=${getOne.body?.data?.user?.email}`
      );

      // 2.d SA PATCH core fields
      const patchCore = await http<StudentProfileResponse>(
        'PATCH',
        `/api/v1/student-profiles/${studentProfileId}`,
        {
          token: saToken,
          body: {
      ...
      record(
        '2',
        'PATCH /student-profiles/:id core fields (sa) → 200',
        patchCore.status === 200,
        `got ${patchCore.status}`
      );

      // 2.e Re-fetch to confirm patch persisted
      const verify = await http<StudentProfileResponse>(
        'GET',
        `/api/v1/student-profiles/${studentProfileId}`,
        { token: saToken }
      );
      record(
        '2',
        'patch persisted: subscriptionPlan=premium, isSeekingJob=true',
        verify.status === 200 &&
          verify.body?.data?.subscriptionPlan === 'premium' &&
          verify.body?.data?.isSeekingJob === true,
        `plan=${verify.body?.data?.subscriptionPlan} seeking=${verify.body?.data?.isSeekingJob}`
      );
    }
    ...
    header('3. Admin — PATCH /:id → 200, DELETE /:id → 403 (super-admin only)');
    {
      if (studentProfileId == null) {
        record('3', 'admin patch/delete checks skipped', false, 'studentProfileId missing');
      } else {
        // Admin CAN list (has student_profile.read)
        const listAdmin = await http<ListResponse>('GET', '/api/v1/student-profiles?pageSize=1', {
          token: adminToken
        });
        record(
          '3',
          'GET /student-profiles (admin) → 200 (has student_profile.read)',
          listAdmin.status === 200,
          `got ${listAdmin.status}`
        );

        // Admin CAN PATCH /:id (has student_profile.update)
        const patchAdmin = await http<StudentProfileResponse>(
          'PATCH',
          `/api/v1/student-profiles/${studentProfileId}`,
          {
            token: adminToken,
            body: { difficultyPreference: 'advanced' }
          }
        );
        record(
          '3',
          'PATCH /student-profiles/:id (admin) → 200 (has student_profile.update)',
        ...
        record(
          '3',
          'DELETE /student-profiles/:id (admin) → 403',
          delAdmin.status === 403,
          `got ${delAdmin.status}`
        );
      }
    ...
    {
      // Student list → 403
      const listStudent = await http('GET', '/api/v1/student-profiles', {
        token: studentToken
      });
      record(
        '4',
        'GET /student-profiles (student) → 403',
        listStudent.status === 403,
        `got ${listStudent.status}`
      );

      // Student POST → 403
      const postStudent = await http('POST', '/api/v1/student-profiles', {
        token: studentToken,
        body: {
          userId: studentUserId,
          enrollmentNumber: `STU-TEST-STUDENT-${RUN_ID}`
        }
      });
      record(
        '4',
        'POST /student-profiles (student) → 403',
      ...
      // Student DELETE → 403
      if (studentProfileId != null) {
        const delStudent = await http('DELETE', `/api/v1/student-profiles/${studentProfileId}`, {
          token: studentToken
        });
        record(
          '4',
          'DELETE /student-profiles/:id (student) → 403',
    ...
    {
      const getMeStudent = await http('GET', '/api/v1/student-profiles/me', {
        token: studentToken
      });
      record(
        '5',
        'GET /student-profiles/me (student) → 403 (no permission)',
    ...
    {
      if (studentProfileId != null) {
        const del = await http('DELETE', `/api/v1/student-profiles/${studentProfileId}`, {
          token: saToken
        });
        record(
          '6',
          'DELETE /student-profiles/:id (sa) → 200',
          del.status === 200,
          `got ${del.status}`
        );

        // Verify 404 after delete
        const after = await http('GET', `/api/v1/student-profiles/${studentProfileId}`, {
          token: saToken
        });
        record(
          '6',
          'GET /student-profiles/:id after delete → 404',
          after.status === 404,
          `got ${after.status}`
        );

        // Parent student user row is still live (hard-delete is profile-only)
        ...
        record(
          '6',
          'parent student user row untouched by profile delete',
    ...
    header('7. Cleanup');
    {
      ...
      if (studentProfileId != null) {
        try {
          await hardDeleteStudentProfile(studentProfileId);
          record('7', 'leftover student profile hard-deleted', true, `id=${studentProfileId}`);
        } catch (err) {
          record(
            '7',
            'leftover student profile hard-deleted',
            false,
        ...
        try {
          await hardDeleteStudentProfileByUserId(studentUserId);
      ...
      if (saUserId != null) {
        try {
          await hardDeleteStudentProfileByUserId(saUserId);
        } catch {
          /* no-op */
        }
      }
      ...
      if (adminUserId != null) {
        try {
          await hardDeleteStudentProfileByUserId(adminUserId);
        } catch {
          /* no-op */
        }
      }
  ...
  if (failed > 0) {
    ...
    console.log('  Stage 6 verdict: \x1b[32mPASS\x1b[0m');
  }
};
