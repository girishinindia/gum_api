-- ══════════════════════════════════════════════════════════════════════════════
-- FUNCTION: udf_category_translations_insert
-- PURPOSE:  Insert a new translation for an existing category. Validates parent
--           category exists, is active, not deleted, and language is valid.
-- RETURNS:  JSONB { success, message, id }
-- ══════════════════════════════════════════════════════════════════════════════

CREATE OR REPLACE FUNCTION udf_category_translations_insert(
...
BEGIN
    ...
    -- ── Verify parent category exists, is active, not deleted ──
    IF NOT EXISTS (
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION 'Translation already exists for category (%) and language (%).', p_category_id, p_language_id;
    END IF;
...
-- SELECT * FROM udf_category_translations_insert(
--     p_category_id := 1,
--     p_language_id := 1,
--     p_name := 'Development'
-- );
