-- ══════════════════════════════════════════════════════════════════════════════
-- FUNCTION: udf_sub_category_translations_update
-- PURPOSE:  Update an existing sub-category translation. Foreign keys
--           (sub_category_id, language_id) are immutable. NULL params leave
--           fields untouched.
-- RETURNS:  JSONB { success, message }
-- ══════════════════════════════════════════════════════════════════════════════

CREATE OR REPLACE FUNCTION udf_sub_category_translations_update(
    p_id                        BIGINT,
...
-- SELECT * FROM udf_sub_category_translations_update(p_id := 1, p_name := 'Updated Name');

-- Test: Update SEO fields
-- SELECT * FROM udf_sub_category_translations_update(p_id := 1, p_meta_title := 'New SEO Title');

-- Test: Deactivate translation
-- SELECT * FROM udf_sub_category_translations_update(p_id := 1, p_is_active := FALSE);
