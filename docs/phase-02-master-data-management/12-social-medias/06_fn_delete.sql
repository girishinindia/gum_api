-- ══════════════════════════════════════════════════════════════════════════════
-- FUNCTION: udf_social_medias_delete
-- PURPOSE:  Soft-delete a social media platform.
--
...
CREATE OR REPLACE FUNCTION udf_social_medias_delete(
    p_id BIGINT
)
RETURNS JSONB
LANGUAGE plpgsql
...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION 'Social media with ID % does not exist or is already deleted.', p_id;
    END IF;
    ...
    RETURN jsonb_build_object(
        'success', TRUE,
        'message', format('Social media %s deleted successfully.', p_id)
    );

EXCEPTION WHEN OTHERS THEN
    RETURN jsonb_build_object(
        'success', FALSE,
        'message', format('Error deleting social media: %s', SQLERRM)
    );
END;
$$;
