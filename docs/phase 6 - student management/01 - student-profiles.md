...
**Request body** — any subset of fields. **Admin-only fields** (403 if student attempts): `subscriptionPlan`, `hasActiveSubscription`, `totalAmountPaid`. Example (student update of learning prefs):
...
Example (admin update of enrollment and subscription):
...
Full updated `StudentProfileDto` in `data`.
