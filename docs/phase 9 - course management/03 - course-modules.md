...
|---|---|---|---|
| 1 | POST — text only (no images) | `application/json` | `{ "courseId": 1, "slug": "getting-started", "displayOrder": 1, "estimatedMinutes": 120, "isActive": true, "translation": { "languageId": 1, "name": "Getting Started", "shortIntro": "Learn the fundamentals" } }` |
| 2 | POST — text + images | `multipart/form-data` | All text fields as form-text, plus optional image files for translation: `icon` (256 px), `image` (512 px), `ogImage` (512 px), `twitterImage` (512 px) (each optional, file) |
| 3 | PATCH — text only (no images) | `application/json` | `{ "displayOrder": 2, "estimatedMinutes": 150 }` |
| 4 | PATCH — text + images | `multipart/form-data` | `displayOrder` = `2` (text), plus subset of translation image files (e.g., `icon`, `image`) |
| 5 | PATCH — images only (no text) | `multipart/form-data` | Only image file parts (e.g., `icon` = `module-icon-256.webp`, `image` = `module-banner.png`) |
...
Update a course module. At least one field is required. Supports both `application/json` and `multipart/form-data`.
...
Create a new translation for a course module. Optionally upload image files for media slots.
...
* **`application/json`** — all text fields as JSON. Image URLs (icon, image, ogImage, twitterImage) cannot be passed as JSON field values; they are set only via the multipart file upload pipeline.
* **`multipart/form-data`** — text fields as form-text, plus optional image files. Image files are placed under form-data field names: `icon` (256 px), `image` (512 px), `ogImage` (512 px), `twitterImage` (512 px). Each image file is re-encoded via Sharp to WebP, max 100 KB, then PUT to Bunny CDN. CDN URLs are returned in corresponding response fields.
...
| # | Variant | Content-Type | Body |
|---|---|---|---|
| 1 | POST — text only (no images) | `application/json` | `{ "languageId": 2, "name": "Primeros Pasos", "shortIntro": "Aprende lo fundamental", "metaTitle": "Primeros Pasos en Desarrollo Web" }` |
| 2 | POST — text + images | `multipart/form-data` | All text fields as form-text, plus optional image files: `icon` (256 px), `image` (512 px), `ogImage` (512 px), `twitterImage` (512 px) (each optional, file) |
| 3 | PATCH — text only (no images) | `application/json` | `{ "name": "Primeros Pasos (Actualizado)", "description": "Este módulo cubre los conceptos básicos del desarrollo web, actualizado para 2026" }` |
| 4 | PATCH — text + images | `multipart/form-data` | `name` = `"Primeros Pasos (Actualizado)"` (text), plus subset of image files (e.g., `icon`, `image`) |
| 5 | PATCH — images only (no text) | `multipart/form-data` | Only image file parts (e.g., `icon` = `module-icon-v2-256.webp`, `image` = `module-banner-v2.png`) |
...
Update a course module translation. At least one field is required. Supports both `application/json` and `multipart/form-data` with the same 4 image slots as POST.
...
Multipart form-data field names for images: `icon` (256 px), `image` (512 px), `ogImage` (512 px), `twitterImage` (512 px).
