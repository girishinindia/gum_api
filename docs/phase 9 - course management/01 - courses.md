...
* **`application/json`** — all text fields as JSON. Image slots (`trailerThumbnail`) cannot be passed as JSON; they are set only via the multipart file upload pipeline.
* **`multipart/form-data`** — text fields and an optional image file. The optional image file goes under the form-data field name **`trailerThumbnail`**. When the file part is present, the server runs the image pipeline: decode via sharp, enforce 512 px square, re-encode to WebP (quality 90, max 100 KB), then PUT the WebP under the deterministic key **`courses/<courseId>-trailer-thumb.webp`**. The CDN URL is returned in the `trailerThumbnailUrl` field of the 201 response.
...
| `trailerThumbnail` | file | no | **Multipart only.** Trailer thumbnail image file (form-data field). Re-encoded to WebP, max 100 KB. |
...
|---|---|---|---|
| 1 | POST — text only (no image) | `application/json` | `{ "code": "WEB101", "slug": "web-development-bootcamp", "difficultyLevel": "bootcamp", "courseStatus": "published", "durationHours": 120, "price": 9999, "currency": "INR", "isFree": false, "isActive": true }` |
| 2 | POST — text + image | `multipart/form-data` | All text fields as form-text (same field names as JSON above), plus `trailerThumbnail` = `web101-thumb-512x512.webp` (file) |
| 3 | PATCH — text only (no image) | `application/json` | `{ "price": 7999, "discountPercentage": 20 }` |
| 4 | PATCH — text + image | `multipart/form-data` | `price` = `7999` (text), `trailerThumbnail` = `web101-thumb-v2.png` (file) |
| 5 | PATCH — image only (no text) | `multipart/form-data` | `trailerThumbnail` = `web101-thumb-final.webp` (file) |
...
Same as [§1.3 POST /courses](#13-post-apiv1courses), but all fields optional (excluding `translation` which is not supported in PATCH). Supports both `application/json` and `multipart/form-data`; image file goes under the form-data field name **`trailerThumbnail`**.

### 5-Variant Body Matrix

| # | Variant | Content-Type | Body |
|---|---|---|---|
| 1 | PATCH — text only (no image) | `application/json` | `{ "price": 7999, "discountPercentage": 20 }` |
| 2 | PATCH — text + image | `multipart/form-data` | `price` = `7999` (text), `trailerThumbnail` = `web101-thumb-v2.png` (file) |
| 3 | PATCH — image only (no text) | `multipart/form-data` | `trailerThumbnail` = `web101-thumb-final.webp` (file) |
...
Create a new translation for a course. Optionally upload image files for media slots.
...
* **`application/json`** — all text fields as JSON. Image URLs (webThumbnail, webBanner, etc.) cannot be passed as JSON field values; they are set only via the multipart file upload pipeline.
* **`multipart/form-data`** — text fields as form-text, plus optional image files. Image files are placed under form-data field names matching the JSON field: `webThumbnail`, `webBanner`, `appThumbnail`, `appBanner`, `videoThumbnail`, `ogImage`, `twitterImage` (7 slots total). Each image file is re-encoded via Sharp to WebP, enforced at 512 px (max 100 KB), then PUT to Bunny CDN under deterministic keys like `course-translations/<translationId>-webThumbnail.webp`. CDN URLs are returned in corresponding response fields.

See **Translation sub-object** table in [§1.3](#13-post-apiv1courses) for field details.
...
| # | Variant | Content-Type | Body |
|---|---|---|---|
| 1 | POST — text only (no images) | `application/json` | `{ "languageId": 2, "title": "वेब डेवलपमेंट बूटकैम्प", "shortIntro": "संपूर्ण स्टैक वेब विकास में महारत हासिल करें", "metaTitle": "वेब डेवलपमेंट बूटकैम्प" }` |
| 2 | POST — text + images | `multipart/form-data` | All text fields as form-text, plus up to 7 image files: `webThumbnail`, `webBanner`, `appThumbnail`, `appBanner`, `videoThumbnail`, `ogImage`, `twitterImage` (each optional, file) |
| 3 | PATCH — text only (no images) | `application/json` | `{ "title": "वेब डेवलपमेंट बूटकैम्प (उन्नत)", "metaDescription": "DevOps के साथ HTML, CSS, JavaScript, React, और Node.js सीखें।" }` |
| 4 | PATCH — text + images | `multipart/form-data` | `title` = `"वेब डेवलपमेंट बूटकैम्प (उन्नत)"` (text), plus subset of image files (e.g., `webThumbnail`, `ogImage`) |
| 5 | PATCH — images only (no text) | `multipart/form-data` | Only image file parts (e.g., `webThumbnail` = `web-thumb-v2.png`, `ogImage` = `og-image-v2.webp`) |
...
Update a course translation. All fields optional. Supports both `application/json` and `multipart/form-data` with the same 7 image slots as POST.
...
Same as [§1.9 POST /translations](#19-post-apiv1coursesidtranslations), but all fields optional. Multipart form-data field names: `webThumbnail`, `webBanner`, `appThumbnail`, `appBanner`, `videoThumbnail`, `ogImage`, `twitterImage`.
