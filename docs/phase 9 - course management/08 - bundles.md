...
Create a new bundle translation. Adds localized content (title, description, SEO metadata, etc.) to a bundle. The bundle must exist and not be deleted. A translation for the same language cannot already exist for this bundle. Optionally upload image files for media slots.
...
* **`application/json`** — all text fields including `bundleId` as JSON. Image URLs (thumbnail, banner, ogImage, twitterImage) cannot be passed as JSON field values; they are set only via the multipart file upload pipeline.
* **`multipart/form-data`** — text fields (including `bundleId`) as form-text, plus optional image files. Image files are placed under form-data field names: `thumbnail` (512 px), `banner` (512 px), `ogImage` (512 px), `twitterImage` (512 px). Each image file is re-encoded via Sharp to WebP, max 100 KB, then PUT to Bunny CDN. CDN URLs are returned in corresponding response fields.
...
|---|---|---|---|
| 1 | POST — text only (no images) | `application/json` | `{ "bundleId": 1, "languageId": 1, "title": "Complete Python Bundle", "description": "Master Python from basics to advanced concepts", "metaTitle": "Complete Python Bundle — Learn Python Online" }` |
| 2 | POST — text + images | `multipart/form-data` | All text fields (including `bundleId`, `languageId`) as form-text, plus optional image files: `thumbnail` (512 px), `banner` (512 px), `ogImage` (512 px), `twitterImage` (512 px) (each optional, file) |
| 3 | PATCH — text only (no images) | `application/json` | `{ "title": "Advanced Python Bundle", "description": "Master advanced Python concepts and patterns" }` |
| 4 | PATCH — text + images | `multipart/form-data` | `title` = `"Advanced Python Bundle"` (text), plus subset of image files (e.g., `thumbnail`, `ogImage`) |
| 5 | PATCH — images only (no text) | `multipart/form-data` | Only image file parts (e.g., `thumbnail` = `bundle-thumb-v2.png`, `banner` = `bundle-banner-v2.webp`) |
...
Update a bundle translation. At least one field must be provided. bundleId and languageId are immutable. Text fields (description, shortDescription, etc.) can be cleared by sending an empty string. Supports both `application/json` and `multipart/form-data` with the same 4 image slots as POST.
...
Multipart form-data field names for images: `thumbnail` (512 px), `banner` (512 px), `ogImage` (512 px), `twitterImage` (512 px).
