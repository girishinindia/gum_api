...
**Content-Type** — `application/json` or `multipart/form-data` (when uploading `profilePhoto` and/or `coverPhoto`).

**Request body** — any combination of the safe fields. Both JSON and multipart/form-data are supported.
...
### Form-data (`multipart/form-data`) — optional file fields

| Field | Type | Notes |
|---|---|---|
| `gender`, `nationality`, `about`, etc. | text | Same safe fields as JSON. |
| `profilePhoto` (aliases: `profilePhotoImage`, `avatar`) | file | PNG / JPEG / WebP, **≤ 100 KB raw**. Processed to WebP, stored at `user-profiles/photos/<id>.webp`. Returns `profilePhotoUrl` in response. |
| `coverPhoto` (aliases: `coverPhotoImage`, `cover`) | file | PNG / JPEG / WebP, **≤ 100 KB raw**. Processed to WebP, stored at `user-profiles/covers/<id>.webp`. Returns `coverPhotoUrl` in response. |

Both `profilePhoto` and `coverPhoto` are optional; include only the files you want to upload.

**Postman examples**

| # | Example name | Content-Type | Body |
|---|---|---|---|
| 1 | Create — JSON (no images) | `application/json` | `{ "gender": "male", "nationality": "Indian", "headline": "Engineer" }` |
| 2 | Create — form-data + profilePhoto + coverPhoto | `multipart/form-data` | `gender` = `male`, `nationality` = `Indian`, `headline` = `Engineer`, `profilePhoto` = `avatar.jpg` (file), `coverPhoto` = `banner.jpg` (file) |

### Responses

#### 201 Created — without photos
...
```json
{
  ...
  "data": {
    ...
    "nationality": "Indian",
    "headline": "Engineer",
    "profilePhotoUrl": "https://cdn.growupmore.com/user-profiles/photos/42.webp",
    "coverPhotoUrl": "https://cdn.growupmore.com/user-profiles/covers/42.webp",
    "createdAt": "2026-04-14T10:00:00.000Z",
    "updatedAt": "2026-04-14T10:00:00.000Z"
  }
}
...
{
  "success": false,
  "message": "File too large: profilePhoto must be ≤ 100 KB raw",
  "code": "BAD_REQUEST"
}
```
...
{
  "success": false,
  "message": "Unsupported media type: expected image/png, image/jpeg, or image/webp",
  "code": "BAD_REQUEST"
}
```
...
**Content-Type** — `application/json` or `multipart/form-data` (when uploading photos).
...
| Field | Type | Notes |
|---|---|---|
| `gender`, `nationality`, `about`, etc. | text | Optional text fields to update. |
| `profilePhoto` (aliases: `profilePhotoImage`, `avatar`) | file | PNG / JPEG / WebP, **≤ 100 KB raw**. Optional. |
| `coverPhoto` (aliases: `coverPhotoImage`, `cover`) | file | PNG / JPEG / WebP, **≤ 100 KB raw**. Optional. |

**Postman examples**

| # | Example name | Content-Type | Body |
|---|---|---|---|
| 1 | Update — JSON (no images) | `application/json` | `{ "headline": "Senior Engineer — mentoring", "themePreference": "light" }` |
| 2 | Update — form-data + text + photos | `multipart/form-data` | `headline` = `Senior Engineer — mentoring`, `profilePhoto` = `new-avatar.jpg` (file), `coverPhoto` = `new-banner.jpg` (file) |
| 3 | Update — form-data, profilePhoto only | `multipart/form-data` | `profilePhoto` = `avatar-v2.jpg` (file) |
...
```json
{
  ...
  "data": {
    ...
    "profilePhotoUrl": "https://cdn.growupmore.com/user-profiles/photos/42.webp",
    "coverPhotoUrl": null,
    "updatedAt": "2026-04-14T11:31:00.000Z"
  }
}
```
...
Same as §1.3 when blocked fields are supplied.
...
**Request body** — `userId` is required; every other field is optional.
...
| Field | Type | Notes |
|---|---|---|
| `userId` | text | Required. The owning user id. |
| `gender`, `nationality`, `aadharNumber`, etc. | text | Optional admin-scoped fields. |
| `profilePhoto` (aliases: `profilePhotoImage`, `avatar`) | file | PNG / JPEG / WebP, **≤ 100 KB raw**. Optional. |
| `coverPhoto` (aliases: `coverPhotoImage`, `cover`) | file | PNG / JPEG / WebP, **≤ 100 KB raw**. Optional. |
...
| # | Example name | Content-Type | Body |
|---|---|---|---|
| 1 | Create — JSON (no images) | `application/json` | `{ "userId": 8, "gender": "female", "nationality": "Indian", "aadharNumber": "XXXX-XXXX-1234" }` |
| 2 | Create — form-data + photos | `multipart/form-data` | `userId` = `8`, `gender` = `female`, `nationality` = `Indian`, `profilePhoto` = `avatar.jpg` (file), `coverPhoto` = `banner.jpg` (file) |
...
```json
{
  ...
  "data": {
    ...
    "gender": "female",
    "nationality": "Indian",
    "profilePhotoUrl": "https://cdn.growupmore.com/user-profiles/photos/43.webp",
    "coverPhotoUrl": "https://cdn.growupmore.com/user-profiles/covers/43.webp",
    "createdAt": "2026-04-14T10:00:00.000Z",
    "updatedAt": "2026-04-14T10:00:00.000Z"
  }
}
...
Partial update. `authorizeSelfOr` lets admins use this on any row and lets students/instructors use it on their own row. Unlike `PATCH /me`, sensitive fields (KYC, bank) are allowed here.
...
| Field | Type | Notes |
|---|---|---|
| `headline`, `bankIfscCode`, etc. | text | Optional fields to update. |
| `profilePhoto` (aliases: `profilePhotoImage`, `avatar`) | file | PNG / JPEG / WebP, **≤ 100 KB raw**. Optional. |
| `coverPhoto` (aliases: `coverPhotoImage`, `cover`) | file | PNG / JPEG / WebP, **≤ 100 KB raw**. Optional. |
...
| # | Example name | Content-Type | Body |
|---|---|---|---|
| 1 | Update — JSON (no images) | `application/json` | `{ "headline": "Admin | GrowUpMore", "bankIfscCode": "HDFC0001234", "profileCompletion": 85 }` |
| 2 | Update — form-data + text + photos | `multipart/form-data` | `headline` = `Admin \| GrowUpMore`, `profilePhoto` = `admin-avatar.jpg` (file), `coverPhoto` = `admin-banner.jpg` (file) |
| 3 | Update — form-data, coverPhoto only | `multipart/form-data` | `coverPhoto` = `banner-v2.jpg` (file) |
...
| `dateOfBirth`, `gender`, `bloodGroup`, `maritalStatus`, `nationality`, `about`, `headline` | Personal info (input fields). |
| `profilePhotoUrl`, `coverPhotoUrl` | CDN URLs — **response-only**, server-set by multipart file uploads. Not accepted as JSON input. |
...
### File upload field aliases

When uploading files via `multipart/form-data`, the following field names are accepted interchangeably:

- `profilePhoto` (canonical) = `profilePhotoImage` = `avatar`
- `coverPhoto` (canonical) = `coverPhotoImage` = `cover`
