...
**Required:** `documentTypeId`, `documentId`, `fileUrl`. Everything else is optional. `expiryDate` must be on or after `issueDate`.

> Only **one** row per `(userId, documentId)` pair — attempting to add a second row for the same document fails with a unique-constraint `BAD_REQUEST`. To replace a file, PATCH the existing row's `fileUrl` instead.
...
```json
{
  ...
  "data": { /* full UserDocumentDto, verificationStatus="pending" */ }
}
```
...
#### 400 Validation error — missing `fileUrl`
...
**Request body** — any subset of the self-create fields. Example:

```json
{
  "fileUrl": "https://cdn.example.com/aadhaar-v2.pdf",
...
> Only `verificationStatus` is accepted on create — the other workflow fields (`verifiedBy`, `verifiedAt`, `rejectionReason`, `adminNotes`) are move-only and can't be set at insert time. Use [§7.8 PATCH](#78-patch-apiv1user-documentsid) to record the verification move itself.

### Responses

#### 201 Created

Same shape as §7.3 — except `verificationStatus` can reflect whatever the admin chose.
...
**Request body — admin verifying a document**
...
**Request body — admin rejecting a document**
