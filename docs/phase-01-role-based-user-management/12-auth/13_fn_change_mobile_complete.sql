-- ============================================================================
-- File: 13_fn_change_mobile_complete.sql
-- Purpose: Complete mobile number change after OTP verification
--          1. Verifies the change request (marks as verified)
--          2. Completes the change (updates users table)
--          3. Invalidates all mobile change OTPs
...
CREATE OR REPLACE FUNCTION udf_auth_change_mobile_complete(
  p_request_id BIGINT
)
RETURNS JSONB
LANGUAGE plpgsql
...
BEGIN
  ...
  -- Complete the change (updates users table with new mobile)
  ...
  -- Invalidate all pending mobile change OTPs for this user
  v_invalidate_result := udf_otp_invalidate(v_user_id, 'change_mobile');
  ...
  -- Revoke all sessions for this user (forces re-login with new mobile)
  ...
  RETURN jsonb_build_object(
    'success', TRUE,
    'message', 'Mobile number changed successfully. All sessions have been revoked. Please log in again.'
  );

EXCEPTION WHEN OTHERS THEN
  RETURN jsonb_build_object(
    'success', FALSE,
    'message', 'Error completing mobile change: ' || SQLERRM
  );
END;
$$;
...
/*
-- Prerequisites:
-- 1. Call udf_auth_change_mobile_initiate to create a request
-- 2. Verify the OTP via app (udf_otp_verify)
-- 3. Then call this function

-- Get latest mobile change request for user 3
-- SELECT id FROM contact_changes WHERE user_id = 3 AND change_type = 'mobile'
-- ORDER BY created_at DESC LIMIT 1;

-- Then complete it (assuming request_id = 5):
-- SELECT udf_auth_change_mobile_complete(5);

-- Verify the mobile was updated
-- SELECT id, mobile, is_mobile_verified, mobile_verified_at FROM users WHERE id = 3;
...
-- SELECT udf_auth_change_mobile_complete(99999);
