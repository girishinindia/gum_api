/**
 * Purpose: Complete forgot password flow - update password, verify channels, revoke sessions, invalidate OTPs
 * Depends: users table, udf_password_history_check(), udf_password_history_add(),
 *          udf_session_revoke_all(), udf_otp_invalidate()
 * Usage: SELECT udf_auth_forgot_password_complete(123, 'newpassword123');
 */

CREATE OR REPLACE FUNCTION udf_auth_forgot_password_complete(
    p_user_id BIGINT,
    p_new_password TEXT
)
RETURNS JSONB
...
BEGIN
    ...
    -- Invalidate forgot_password OTPs
    v_otp_invalidate_result := udf_otp_invalidate(p_user_id, 'forgot_password');
...
/*
SELECT udf_auth_forgot_password_complete(5, 'NewSecurePassword123');
